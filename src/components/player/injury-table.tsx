    ...
    <section aria-labelledby="injury-history-heading">
      ...
      {injuries.length === 0 ? (
        <p className="mt-4 text-sm text-ink-dim">
          Nothing logged. Not the same as nothing happening - if he has been
          carrying something, it is not in here yet.
        </p>
      ) : (
          ...
          <table className="w-full text-[12.5px] border-collapse">
            <thead>
              <tr className="text-left">
                {["region", "side", "severity", "occurred", "back", "days"].map(
                  (h) => (
                    <th
                      key={h}
                      scope="col"
              ...
              {injuries.map((injury) => {
                ...
                return (
                  <tr
                    ...
                    style={
                      current
                        ? { boxShadow: "inset 2px 0 0 0 var(--out)" }
                        : undefined
                    }
                  >
                    <td
                      className={`py-2.5 pr-3 ${current ? "pl-3 text-ink font-semibold" : "text-ink"}`}
                    >
                      {REGION_LABEL[injury.body_region]}
                    </td>
                    <td className="py-2.5 pr-3 text-ink-dim">{injury.side}</td>
                    <td
                      className={`py-2.5 pr-3 ${current ? "text-out" : "text-ink-dim"}`}
                    >
                      {injury.severity}
                    </td>
                    <td className="num py-2.5 pr-3 text-ink-dim whitespace-nowrap">
                      {formatDate(injury.occurred_on)}
                    </td>
